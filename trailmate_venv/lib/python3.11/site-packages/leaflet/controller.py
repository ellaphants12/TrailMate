...
class WrappedSocket(object):
    ...
    def accept(self, *args, **kwargs):
        human_error = (
            "There is nothing to accept. Just use %s.recv and strip the SAM response."
            % self.__class__.__name__
        )
        raise AttributeError(human_error)

    def connect(self, *args, **kwargs):
        human_error = "This %s instance is connected to %s." % (
            self.__class__.__name__,
            repr(self.sam_api),
        )
    ...
    def create_dest(self, sam_sock, name=None, sock_type="stream", i2cp=None):
        if sock_type not in ("stream",):
            raise NotImplementedError(
                "Socket type %s is not implemented" % repr(sock_type)
            )
        if not name:
...
class OurDest(samtools.Dest):
    ...
    def __init__(self, controller, sock_type, nickname, i2cp_options):
        ...
        keyfile = samtools.session_create(
            sock, sock_type, sig_type, nickname, i2cp_options
        )
        self.sam_sock = sock
