...
def sam_readline(sock, partial=None):
    ...
    while True:
        try:
            ...
            if not c:
                raise EOFError(
                    "SAM connection died. Partial response %r %r" % (partial, response)
                )
            elif c == b"\n":
                break
            else:
...
def session_create(sock, sock_type, sig_type, name, i2cp_options=None):
    ...
    cmd = "SESSION CREATE STYLE=%s DESTINATION=TRANSIENT SIGNATURE_TYPE=%d ID=%s " % (
        sock_type,
        sig_type,
        name,
    )
...
def stream_connect(nickname, dest):
    line = yield (
        "STREAM CONNECT ID=%s DESTINATION=%s SILENT=false" % (nickname, dest.base64)
    )
...
def pack_datagram(data, max_version, nickname, dest, options):
    header = (
        " ".join((max_version, nickname, dest.base64, join_kv(options))).rstrip() + "\n"
    )
...
class Dest(object):
    ...
    def __init__(self, keyfile, encoding, sig_type=None, private=False):
        ...
        if private:
            if sig_type not in self.sskey_len_dict.keys():
                raise NotImplementedError(
                    "Signing key type %s not implemented" % repr(sig_type)
                )
        ...
        if private:
            ...
            self.signing_secret_key = self.__class__.read_signing_secret_key(
                keyfile, sig_type
            )
    ...
    def read_keys_cert(cls, keyfile):
        fixed_len = 256 + 128 + 3
        keys_cert_header = cls._read(
            keyfile, fixed_len, throw="KeysAndCert header truncated"
        )
    ...
    def read_signing_secret_key(cls, keyfile, sig_type):
        ...
        secret_signing = cls._read(
            keyfile, key_len, throw="Secret signing key truncated"
        )
