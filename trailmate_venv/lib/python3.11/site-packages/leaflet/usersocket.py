...
class DatagramSocket(WrappedSocket):
    ...
    def transmit(self, *args):
        ...
        real_data = pack_datagram(
            data, self.controller.max_version, self.name, dest, {}
        )
    ...
    def _datagram_collect(self, bufsize, *args):
        ...
        if address != self.dgram_api:
            raise SourceError(
                "Packet src=%r not from SAM UDP API %r" % (address, self.dgram_api)
            )
